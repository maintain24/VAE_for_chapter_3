...
class Conv_Encoder(nn.Module):
    ...
    def forward(self, x, labels):
        ...
        # 扩展标签以匹配特征数据的形状
        labels_expanded = labels.unsqueeze(-1).expand(-1, x.size(1)).unsqueeze(-1)
        # 拼接标签和特征数据
        x = torch.cat((x, labels_expanded), -1)
...
class GRU_Encoder(nn.Module):
    ...
    def forward(self, x, labels):
        ...
        x = self.relu(x[:, -1, :])
        x = self.fc_1(x)
        print('x.shape:', x.shape)  # torch.Size([32, 435])
        return x


class GRU_Decoder(nn.Module):
    def __init__(self, vocab_size, batchsize, latent_dim, num_layers=5):  # 增加了, num_layers=3
        ...
        # self.gru = nn.GRU(batchsize, latent_dim, num_layers, batch_first=True)  # 原来的维度是nn.GRU(501, vocab_size)
        ...
        self.vocab_size = vocab_size
        self.latent_to_hidden = nn.Linear(batchsize * 2, batchsize)

    def forward(self, z, hidden, labels):
        batch_size = z.shape[0]
        # hidden = torch.zeros(self.num_layers, batch_size, self.latent_dim).to(z.device)  # hidden添加新的初始化方式

        # 将标签数据整合到潜在向量中
        # labels = labels.unsqueeze(-1)  # 将labels的形状从 [batch_size] 变为 [batch_size, 1]
        # labels = labels.unsqueeze(-1).expand(batch_size, -1)
        # print('z weidu:', z.shape)  # torch.Size([32])
        # print('labels weidu:', labels.shape)  # torch.Size([32, 1])
        z = torch.cat((z, labels), dim=-1)  # 拼接潜在向量和标签  # torch.Size([64]) (z + labels)
        z = self.latent_to_hidden(z)  # 从拼接的潜在向量和标签映射到隐藏层维度  torch.Size([290])

        z = torch.clamp(z, min=0, max=self.vocab_size - 1).long()  # 限制张量的值在一个指定的范围内
        z = self.embedding(z.long()).unsqueeze(1)  # torch.Size([290, 1, 290])
...
class GRU_Decoder_2(nn.Module):
    ...
    def forward(self, z, max_length, labels):
        batch_size = z.size(0)
        # 扩展标签以匹配潜在空间的维度
        labels_expanded = labels.unsqueeze(-1).repeat(1, self.latent_dim)

        # 拼接潜在向量和标签
        z = torch.cat((z, labels_expanded), -1)

        # 将潜在向量转换为GRU的初始隐藏状态
        hidden = self.latent_to_hidden(z)
        hidden = hidden.view(self.num_layers, batch_size, self.latent_dim)

        # 初始化解码器的输入序列
        input = torch.zeros(batch_size, 1, dtype=torch.long).to(z.device)
        outputs = []
...
class Molecule_VAE(nn.Module):
    ...
    def forward_decoder(self, Z, x, labels):
        ...
        input = torch.ones(batch_size).to(self.device)  # 原本有* 2  但为什么要乘2呢？
        ...
        for t in range(trg_len):  # (1,120)
            output, hidden = self.decoder(input, hidden, labels)
            ...
            if random.random() < self.teacher_forcing_ratio:
                ...
                input = x[:, t, :].argmax(1)  # 为什么不是argmax(2)？  改成累积
                output = x[:, t, :] # 为什么不是argmax(2)？  改成累积
                # print('每个batchsize的target_input:', input)
                # print('GRU_decoder_input:', input)  # 用来查看（batchsize，1）的张量，打算把这里改成性能标签且累积
            ...
            # print('每个batchsize的target_output:', output)
            outputs[:, t:t + 1, :] = output_expanded
            # print('outputs[:, t:t + 1, :]:', outputs[:, t:t + 1, :])
    ...
    def forward(self, x, labels):
        ...
        self.h_enc = self.encoder(x, labels)
        z = self._sample_latent(self.h_enc)

        recon_x = self.forward_decoder(z, x, labels)
        return recon_x, z
