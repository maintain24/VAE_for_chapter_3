...
def main():
    ...
    if use_vae:
        ...
        # input_channels = 120  # 对应Conv_Encoder
        # enc = Conv_Encoder(vocab_size, input_channels=input_channels).to(device)
        input_channels = X_train.shape[-1]  # 对应GRU_Encoder 110
    ...
    for epoch in range(args.epochs):
        ...
        for i in range(5):  # 取决于想打印前几列
            ...
            # 记录索引，保存
            inputs_index = tenser_to_list(inputs[i])
            outputs_index = tenser_to_list(outputs_onehot)
            save_smiles_to_csv(inputs_index, outputs_index, r'/mnt/pycharm_project_VAE/tensors_index.csv')
        ...
        # 调整 inputs 的形状，保持 batch 维度，将其它维度展平
            ...
            input_recon_val = model(inputs_flat_val)
            latent_loss_val = latent_loss(model.z_mean, model.z_sigma)
            # 原本是loss = F.binary_cross_entropy(input_recon, inputs, size_average=False) + latent_loss_val
