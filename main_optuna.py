...
def train_with_params(trial):
    # 从 trial 对象获取超参数
    latent_dim = trial.suggest_int('latent_dim', 100, 400)  # 292
    batch_size = trial.suggest_int('batch_size', 32, 128)
    # batch_size = 32
    lr = trial.suggest_loguniform('lr', 1e-5, 1e-2)  # 1e-5, 1e-2
    epochs = trial.suggest_int('epochs', 2, 50)  # 5 或其他固定值

    data = pd.read_csv(args.data, nrows=1000)  # 取前2000-4000行，总共50000行
    ...
    if use_vae:
        ...
        dec = GRU_Decoder(vocab_size, batch_size, latent_dim, num_layers=3).to(device)  # 增加了, num_layers=3
    ...
    x_train_data_per_epoch = X_train.shape[0] - X_train.shape[0] % batch_size
    x_val_data_per_epoch = X_test.shape[0] - X_test.shape[0] % batch_size
    ...
    for epoch in range(epochs):
        ...
        if last_epoch_inputs is not None and last_epoch_input_recon is not None:
            inputs_numpy = last_epoch_inputs.numpy()
            input_recon_numpy = last_epoch_input_recon.numpy()

            # 查看输入和输出的维度
            print("Inputs Dimension:", inputs_numpy.shape)
            print("Input Recon Dimension:", input_recon_numpy.shape)

            inputs_df = pd.DataFrame(inputs_numpy)
            input_recon_df = pd.DataFrame(input_recon_numpy)
            combined_df = pd.concat([inputs_df, input_recon_df], axis=1)

            combined_df.to_csv(f'{output_dir}/inputs_and_recon_last_epoch.npz', index=False)

        # 在最后一个epoch结束时进行可视化
        if epoch == epochs - 1:  # args.epochs
            plot_index_list_prediction(all_inputs, all_outputs,
                                       r'/mnt/pycharm_project_VAE/index_fig/All_Figure_{}.jpg'.format(current_date))
        ...
        for i in range(25):  # 取决于想打印前几列
            ...
            plot_index_list_prediction(inputs_index, outputs_index,
                                       r'/mnt/pycharm_project_VAE/index_fig/{}Figure_{}.jpg'.format(i, current_date))
    ...
    # 计算所有epoch的平均损失值，然后返回唯一指标给optuna寻优
    optuna_loss = sum(all_losses) / len(all_losses)
